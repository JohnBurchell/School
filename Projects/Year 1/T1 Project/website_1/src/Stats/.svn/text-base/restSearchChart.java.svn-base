...
public class restSearchChart extends DataSourceServlet {
  ...
  public DataTable generateDataTable(Query query, HttpServletRequest request) {
    ...
    //Columns, these are the headers under which the data is stored, as the pie chart
    //I wish to use only has 2 columns, i only create 2 here.
    ArrayList<ColumnDescription> cd = new ArrayList<ColumnDescription>();
    //Add names to the columns.
    cd.add(new ColumnDescription("Name", ValueType.TEXT, "Restaurant name"));  
   ...
   * It queries the statsRestaurants table in the database and returns the values required<br>
  ...
  public void getDataForTable() {
	  ...
	  try {			
		  ...
		  //Connections are made and queries performed.
			...
			String QueryString = "select * from statsrestaurants";
			rs = stmt.executeQuery(QueryString);		
			
			while (rs.next()) {
				
				String restName = rs.getString("restName");
				int numOfSearches = (Integer.parseInt(rs.getString("numOfSearches")));
