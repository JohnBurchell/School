...
int main(int argc, char *argv[]){ 
	...
	while(loop) {
		...
		case 1: 
			...
			//Same as below, re-alloc space
			ppost = malloc(sizeof(PERSON)); 
			break;
		case 2: 
			*ppost = input_record(); 
			append_file(ppost);
			//Re-malloc space as it was free'd earlier
			ppost = malloc(sizeof(PERSON)); break;
		case 3: search(); break;
...
PERSON input_record(){
	...
	char firstName[sizeof(input->firstName)], 
		familyName[sizeof(input->familyName)], 
	...
	//Assign memory to the size of the array which will store them.
	//valgrind didn't like me copying to unknown space!
